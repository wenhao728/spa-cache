...
from spac.dream import SpaCachedForwardHooks, register_hooks, sample, sample_spa_cache
...
class Dream(LM):
    def __init__(
        ...
        if cache == "spa":
            ...
            register_hooks(
                self.model,
                "model.layers",
                block_forward_fn=forward_hooks.block_forward,
                sample_fn=partial(sample_spa_cache, log_ttft=False),
            )
            SequenceMgmt.set_hparams(
                ...
                n_warmup_steps=cache_warmup_steps,
                # tokenizer=self.tokenizer,
            )
        elif cache is not None:
            raise NotImplementedError(f"Cache type {cache} not implemented.")
        
    def _set_log_ttft(self, enable: bool):
        if self.cache is None:
            register_hooks(
                self.model, transformer_blocks_name=None,  # skip transformer blocks forward hooks
                sample_fn=partial(sample, log_ttft=enable),
            )
        elif self.cache == "spa":
            register_hooks(
                self.model, transformer_blocks_name=None,  # skip transformer blocks forward hooks
                sample_fn=partial(sample_spa_cache, log_ttft=enable),
            )
        else:
            raise NotImplementedError(f"Cache type {self.cache} not implemented.")
    ...
    def generate_until(self, requests: List[Instance]) -> List[str]:
        ...
        for batch_id, batch in enumerate(
            ...
            # inputs
            inputs = self._batch_encode(batch["input_text"])
            input_ids = inputs["input_ids"].to(self.device)
            attn_mask = None
            if not self.skip_attn_mask:
                attn_mask = inputs["attention_mask"].to(self.device)

            # ttft
            if self.log_ttft > 0:
                if batch_id == self.log_warmup:
                    self._set_log_ttft(enable=True)
                elif batch_id == self.log_warmup + self.log_ttft:
                    self._set_log_ttft(enable=False)
            ...
            with context:
                output_dict = self.model.diffusion_generate(
                    input_ids, attention_mask=attn_mask, **self.generate_kwargs
                )

            output_ids = output_dict.sequences[:, input_ids.shape[1] :]

            # log latency
            if self.log_ttft > 0 and (self.log_warmup <= batch_id < self.log_warmup + self.log_ttft):
                total_ttft += output_dict.ttft
            ...
            try:
                ...
                logger.error(f"Decoding error: {e}, trying row by row decoding.")
                output_text = []
                for row_idx in range(output_ids.shape[0]):
                    try:
                        output_text.append(
                            self.tokenizer.batch_decode(
                                output_ids[row_idx : row_idx + 1, :],
                                skip_special_tokens=self.escape_until,
                            )[0]
                        )
                    except Exception as e:
                        logger.error(f"Decoding error at row {row_idx}: {e}")
                        output_text.append("# invalid decoding")
        ...
        n_outputs = len(outputs) - (self.log_warmup + self.log_ttft) * self.batch_size
